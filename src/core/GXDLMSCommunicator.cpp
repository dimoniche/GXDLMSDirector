...
int GXDLMSCommunicator::readDLMSPacket(CGXByteBuffer &data, CGXReplyData &reply)
{
    ...
    do {
        ...
        if ((ret = readBytes(bb, eop)) != 0)
            return ret;
    } while ((ret = m_client->GetData(bb, reply, notify)) == DLMS_ERROR_CODE_FALSE);
...
int GXDLMSCommunicator::readDataBlock(CGXByteBuffer &data, CGXReplyData &reply)
{
    ...
    reply.Clear();
    do {
        if ((ret = readDLMSPacket(data, reply)) != 0)
            return ret;
        data.Clear();
        if (reply.IsMoreData()) {
            if ((ret = m_client->ReceiverReady(reply, data)) != 0)
                return ret;
        }
...
int GXDLMSCommunicator::readDataBlock(std::vector<CGXByteBuffer> &data, CGXReplyData &reply)
{
    for (auto &packet : data) {
        const int ret = readDataBlock(packet, reply);
        if (ret != 0)
            return ret;
    }
    return DLMS_ERROR_CODE_OK;
...
int GXDLMSCommunicator::initializeConnection()
{
    ...
    if (ret != 0) {
        emit errorOccurred(tr("Failed to open media connection."));
        return ret;
    }
...
int GXDLMSCommunicator::getAssociationView()
{
    ...
    if ((ret = readDataBlock(data, reply)) != 0)
        return ret;
    if ((ret = m_client->ParseObjects(reply.GetData(), true)) != 0)
        return ret;
    ...
    emit traceMessage(tr("Association view loaded: %1 objects")
                          .arg(static_cast<int>(m_client->GetObjects().size())));
    return DLMS_ERROR_CODE_OK;
}
